...
<!--      background-color: rgba(0,0,0,0.2)">自媒体创意素材网站</div>-->
    <div style="margin: 100px auto; background-color: rgba(245,239,239,0.8); width: 350px; height: 385px; padding: 20px; border-radius: 10px">
      <div style="margin: 20px 0; text-align: center; font-size: 24px;color:rgb(244,208,212)"><b>欢迎登录</b></div>
      <el-form :model="user" :rules="rules" ref="userForm">
        <el-form-item v-if="user.loginType === 'account'">
          ...
          <el-form-item>
            <div style="display: flex">
              <el-input placeholder="请输入验证码" size="mid" v-model="code" style="width: 200px"></el-input>
          ...
          <el-form-item style="margin: 10px 0; text-align: center">
            <el-button style="width: 130px;background-color: rgb(244,208,212);border-color:rgb(244,208,212)" type="primary" size="medium"  autocomplete="off" @click="login">登录</el-button>
            <el-button type="text" size="medium" style="background-color: #6d9d88; font-size: 14px; width: 130px; color: white" autocomplete="off" @click="$router.push('/register')">注册</el-button>
...
export default {
  ...
  data() {
    return {
      loginType: 'account',  // 默认选择账号登录
      user: {loginType: 'account'}, // 初始化时设置为账号登录
      pass: {},
      code: '',
  ...
  methods: {
    ...
    loginPhone() {
      ...
      this.$refs['userForm'].validate((valid) => {
        if (valid) {  // 表单校验合法
          this.request.post("/user/loginPhone", this.user).then(res => {
            if(res.code === '200') {
              localStorage.setItem("user", JSON.stringify(res.data))  // 存储用户信息到浏览器
              localStorage.setItem("menus", JSON.stringify(res.data.menus))  // 存储用户信息到浏览器
